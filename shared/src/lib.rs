...
pub fn parse(bytes : &[u8]) -> Result<ParsedTree, DataParseError> {
    ...
    loop {
        ...
        let data = match DataIDs::try_from(bytes[typeidx]).ok().ok_or(DataParseError{message : "Attempted to parse invalid datatype".to_string()})?{
            DataIDs::U8 | DataIDs::U16 | DataIDs::U32 | DataIDs::U64 | DataIDs::I8 | DataIDs::I16 | DataIDs::I32 | DataIDs::I64 | DataIDs::F32 | DataIDs::F64 | DataIDs::CHAR => {
            ...
            DataIDs::STRING => {
                ...
                if bytes.len() >= idx {
                    let string = match from_utf8(&bytes[idx..length]) {
                            Ok(t) => t.to_string(),
                            Err(e) => return Err(DataParseError {message : "String wasn't a valid utf8 string".to_string()})
                        };
                    ParsedData::STRING(string)
                }
                else{
                    return Err(DataParseError { message: "Client lied about string length".to_string() })
                }
            },
            DataIDs::ENDPKG => {
                return Ok(tree)
...
fn parse_basic(bytes : &[u8]) -> Result<(ParsedData, usize), DataParseError> {
    let mut idx = 0;
    let datatypeerr = "Error when parsing ".to_string();
    let data = match DataIDs::try_from(bytes[0]).ok().ok_or(DataParseError{message : "Attempted to parse invalid datatype".to_string()})?{
        DataIDs::U8 => {
            let parsed = ParsedData::U8(u8::from_be_bytes(*bytes[1..].first_chunk::<1>().ok_or(
                    DataParseError{message : format!("{datatypeerr} U8")})?));
            idx += 1;
            return Ok((parsed,idx));
        },
        DataIDs::U16 => {
            let parsed = ParsedData::U16(u16::from_be_bytes(*bytes[1..].first_chunk::<2>().ok_or(
                    DataParseError{message : format!("{datatypeerr} U16")})?));
            idx += 2;
            return Ok((parsed,idx));
        },
        DataIDs::U32 => {
            let parsed = ParsedData::U32(u32::from_be_bytes(*bytes[1..].first_chunk::<4>().ok_or(
                    DataParseError{message : format!("{datatypeerr} U32")})?));
            idx += 4;
            return Ok((parsed,idx));
        },
        DataIDs::U64 => {
            let parsed = ParsedData::U64(u64::from_be_bytes(*bytes[1..].first_chunk::<8>().ok_or(
                    DataParseError{message : format!("{datatypeerr} U64")})?));
            idx += 8;
            return Ok((parsed,idx));
        },
        DataIDs::I8 => {
            let parsed = ParsedData::I8(i8::from_be_bytes(*bytes[1..].first_chunk::<1>().ok_or(
                    DataParseError{message : format!("{datatypeerr} I8")})?));
            idx += 1;
            return Ok((parsed,idx));
        },
        DataIDs::I16 => {
            let parsed = ParsedData::I16(i16::from_be_bytes(*bytes[1..].first_chunk::<2>().ok_or(
                    DataParseError{message : format!("{datatypeerr} I16")})?));
            idx += 2;
            return Ok((parsed,idx));
        },
        DataIDs::I32 => {
            let parsed = ParsedData::I32(i32::from_be_bytes(*bytes[1..].first_chunk::<4>().ok_or(
                    DataParseError{message : format!("{datatypeerr} I32")})?));
            idx += 4;
            return Ok((parsed,idx));
        },
        DataIDs::I64 => {
            let parsed = ParsedData::I64(i64::from_be_bytes(*bytes[1..].first_chunk::<8>().ok_or(
                    DataParseError{message : format!("{datatypeerr} I64")})?));
            idx += 8;
            return Ok((parsed,idx));
        },
        DataIDs::CHAR => {
            let parsed = ParsedData::CHAR(char::from_u32(u32::from_be_bytes(*bytes[1..].first_chunk::<4>().ok_or(
                    DataParseError{message : format!("{datatypeerr} CHAR")})?)).unwrap());
            idx += 4;
            return Ok((parsed,idx));
        },
        DataIDs::F32 => {
            let parsed = ParsedData::F32(f32::from_be_bytes(*bytes[1..].first_chunk::<4>().ok_or(
                    DataParseError{message : format!("{datatypeerr} F32")})?));
            idx += 4;
            return Ok((parsed,idx));
        },
        DataIDs::F64 => {
            let parsed = ParsedData::F64(f64::from_be_bytes(*bytes[1..].first_chunk::<8>().ok_or(
                    DataParseError{message : format!("{datatypeerr} F64")})?));
            idx += 8;
            return Ok((parsed,idx));
        },
        _ => {
            return Err(DataParseError{message : "Tried to parse non-basic datatype as a basic datatype".to_string()})
        }
...
mod tests {
    ...
    fn test_encoding_string(){
        let  TESTS_String : [String; 5] = ["Räkmacka".to_string(),"I LOVE 'Escaping' \"STRINGS\"".to_string(), "I ^^, *** Sp3<!4L <h4r4<t3r2".to_string(), "This is straight up just a normal string".to_string(), fs::read_to_string("./testcase.txt").expect("I can't see your extremely funny text file test case :(")];
